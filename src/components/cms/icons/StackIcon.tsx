/**
 * Framework/stack marks (Next.js, Astro) rendered in a single monochrome
 * `currentColor` path so they adapt to the theme automatically — dark on light
 * surfaces, white on dark. Set the color with a text-* utility on the wrapper.
 *
 * Astro path is the official press-kit icon (astro.build/press), recolored to
 * currentColor. Next.js is a clean monochrome "N-in-a-circle" mark.
 */

import type { ReactNode } from "react";
import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from "@/components/ui/tooltip";

export type StackKey = "nextjs" | "astro";

export function NextjsIcon({ className = "" }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" className={className} aria-hidden="true">
      <circle cx="12" cy="12" r="10.25" strokeWidth="1.4" />
      <path d="M8.7 8 V16" strokeWidth="1.7" strokeLinecap="round" />
      <path d="M8.7 8 L15.3 16.4" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round" />
      <path d="M15.3 8 V12.7" strokeWidth="1.7" strokeLinecap="round" />
    </svg>
  );
}

export function AstroIcon({ className = "" }: { className?: string }) {
  return (
    <svg viewBox="0 0 85 107" fill="currentColor" className={className} aria-hidden="true">
      <path d="M0 69.5866C0 69.5866 14.3139 62.6137 28.6678 62.6137L39.4901 29.1204C39.8953 27.5007 41.0783 26.3999 42.4139 26.3999C43.7495 26.3999 44.9325 27.5007 45.3377 29.1204L56.1601 62.6137C73.1601 62.6137 84.8278 69.5866 84.8278 69.5866C84.8278 69.5866 60.5145 3.35233 60.467 3.21944C59.7692 1.2612 58.5911 0 57.0029 0H27.8274C26.2392 0 25.1087 1.2612 24.3634 3.21944C24.3108 3.34983 0 69.5866 0 69.5866Z" />
      <path d="M27.5894 91.1365C22.7555 86.7178 21.3444 77.4335 23.3583 70.7072C26.8503 74.948 31.6888 76.2914 36.7005 77.0497C44.4375 78.2199 52.0359 77.7822 59.2232 74.2459C60.0454 73.841 60.8052 73.3027 61.7036 72.7574C62.378 74.714 62.5535 76.6892 62.318 78.6996C61.7452 83.5957 59.3086 87.3778 55.4332 90.2448C53.8835 91.3916 52.2437 92.4167 50.6432 93.4979C45.7262 96.8213 44.3959 100.718 46.2435 106.386C46.2874 106.525 46.3267 106.663 46.426 107C43.9155 105.876 42.0817 104.24 40.6845 102.089C39.2087 99.8193 38.5066 97.3081 38.4696 94.5909C38.4511 93.2686 38.4511 91.9345 38.2733 90.6309C37.8391 87.4527 36.3471 86.0297 33.5364 85.9478C30.6518 85.8636 28.37 87.6469 27.7649 90.4554C27.7187 90.6707 27.6517 90.8837 27.5847 91.1341L27.5894 91.1365Z" />
    </svg>
  );
}

export function StackIcon({ stack, className }: { stack: StackKey; className?: string }) {
  return stack === "astro" ? <AstroIcon className={className} /> : <NextjsIcon className={className} />;
}

export const STACK_LABEL: Record<StackKey, string> = {
  nextjs: "Next.js",
  astro: "Astro",
};

/**
 * Wraps a stack mark in a subtle hover tooltip that spells out the framework,
 * for people who don't recognise the icon. Self-contained provider so it works
 * anywhere without a global TooltipProvider. Pass the icon (or a chip around it)
 * as children.
 */
export function StackTag({ stack, children }: { stack: StackKey; children: ReactNode }) {
  return (
    <TooltipProvider delayDuration={150}>
      <Tooltip>
        <TooltipTrigger asChild>{children}</TooltipTrigger>
        <TooltipContent side="top">Built with {STACK_LABEL[stack]}</TooltipContent>
      </Tooltip>
    </TooltipProvider>
  );
}
