...
> **Stack reconciliation (2026-07-12).** §35–37 were rewritten to match the team's **actual** production stack (Linear "Internal Tech Stack & Costs", updated 2026-07-08, + the repos' `TECH_STACK.md`): **Bun + Hono 4 + Zod 4**, **Drizzle ORM on self-hosted Postgres** (pgvector, RLS), **Better Auth**, **Lexical + isomorphic-dompurify**, **Typesense**, **Cloudflare R2 + sharp**, **ZeptoMail**, **Dodo Payments** (Merchant of Record), **Anthropic / Claude Agent SDK**, **Valkey**, **Cloudflare Workers**, all on **one self-hosted VPS** with **GlitchTip** — a low, flat fixed monthly baseline (exact internal cost figures are kept out of this document). **§35 is authoritative.** A few illustrative "production would need X" notes in the earlier inventory/roadmap sections still name generic alternatives (Stripe, Liveblocks, Resend, managed cloud); read those through §35 — the real choices are Dodo, Valkey + WebSocket, ZeptoMail, and one self-hosted box.
...
**Security posture.** The audit found concrete items for the security backlog: the visual editor's rich inline editing commits raw `innerHTML` (stored-XSS vector; the block editor's `sanitizeInlineHtml` shows the fix pattern but misses `javascript:` hrefs — the production stack's **isomorphic-dompurify** is the sanitizer that closes it, §16), share-preview tokens are self-encoded rather than server-issued, client-side role gates have no server counterpart yet, and the demo guest door bypasses auth entirely by design. §8 carries the full checklist; none of these are surprising for a prototype, all are must-fix before GA.

**The plan in numbers** (assumptions labeled in §37–38): a unified Postgres design of ~45 tables (§11); ~90 application endpoints plus a public delivery API with markdown content negotiation (§13–14); a 20-tool MCP server with governance enforcement (§15); Typesense-backed search per SEARCH_PLAN.md (§21); ~94 engineer-weeks across 14 epics with a 4→6 engineer team, GA in roughly 6–7 months. **On infrastructure cost, §35–37 are reconciled against the team's real stack** (Linear "Internal Tech Stack & Costs"): the architecture is one self-hosted VPS + Cloudflare edge + usage-metered APIs — a **low, flat fixed monthly baseline**, far below what a managed-cloud model would imply, so infra is a rounding error against engineering burn. The first 20 tickets are fully written in §40 and the strangler path from demo to production closes the document in §44.
...
- Notion-style entry **BlockEditor**: two-level slash menu (Featured + Basic, drilling into AI / Components / Embeds with scoped search); 20+ block types (headings, lists, to-do, quote, callout with tones, code, table, toggle, button, bookmark, divider); AI commands with prompt inputs (write, continue, summarize, improve, longer/shorter, generate image); reusable component instances; embeds (YouTube, Vimeo, Loom, Figma, CodePen, CodeSandbox, Spotify, generic); a full image block (media-library picker, upload, paste URL, caption, alt text, alignment, hyperlink, crop); fully editable component instances (Figma-style per-use overrides that never touch the master, form + inline editing, duplicate, reset); a sandboxed code/HTML embed block and native video (MP4) players; Markdown paste that preserves hyperlinks, headings and lists by reading the clipboard's `text/html` (Notion/Docs); a Markdown view/edit toggle; inline bold/italic, H1-H6 turn-into, and a full **hyperlink editor** (URL or existing-page picker, opens in a new tab with `rel="noopener"`, `javascript:` schemes blocked) from the text-selection toolbar; per-paragraph presence avatars.
...
- **Agent instructions** (`instructions-store.ts` + `/instructions` page, modeled on Webflow AI agent instructions and Sanity Agent Skills — see INSTRUCTIONS_PLAN.md): a **workspace-level library of Skills** (teach the agent how to do specific tasks) **and Rules** (boundaries respected on every run), authored once and shared with every project and teammate — the standardization/reuse layer. Template gallery (Brand voice, Editorial style, SEO writing, Release notes / SEO defaults, Naming, Accessibility, Localization), blank create, and `.md` import/export (Agent Skills standard interop). The editor's **Insert reference** menu drops live CMS entities — media assets (images/video/files, picked from the media library with thumbnails), collections, fields, components, sections, pages, brand tokens, other skills/rules — organized as a Webflow-style nested drill-down — into the markdown as `@[Type: Label]` chip tokens, rendered as atomic non-editable chips in an editable Notion-style Text view (with a raw Markdown view alongside). Per-instruction On/Off plus a per-project "Applies to this project" opt-out. Every agent run records which instructions it followed (`AgentRun.instructions`) and streams a "Following N workspace instructions" step into the thread; the Agent page shows the live count. Production: instructions become backend documents (versioning/approval via the content pipeline), are injected into run context server-side, are served to external MCP agents, and sync to a git repo for org-plugin distribution.
...
- **Markdown, both directions:** pasting multi-line Markdown (Notion copies as Markdown; `looksLikeMarkdown` heuristic) converts to real blocks via `markdownToBlocks` — replacing the current empty paragraph or inserting after, with a "Markdown converted · N blocks added" toast. The paste handler reads the clipboard's `text/html` first and, when it carries links or structure, runs it through `htmlToMarkdown` so hyperlinks, headings, lists and emphasis survive as blue underlined anchors instead of flattening to plain text. A hover **Markdown button** (top-right) flips the whole document to a mono textarea via `blocksToMarkdown`; edits commit back through `markdownToBlocks` on exit ("Rich text" button). Dialect notes surfaced in the UI: a bare URL on its own line becomes an embed; `> [!info]` becomes a callout.
...
- **New model** → template gallery (`SCHEMA_TEMPLATES`, 20 ready-made shapes: Blog post, Testimonial, Author, Case study, Glossary term, White paper, Product, Job posting, Event, Documentation, API reference, Course, Recipe, Video, Podcast episode, Music track, Portfolio project, Webinar, Comparison page, FAQ, or blank) → `modelActions.add(projectId, model)` with `newModelId()`. Each template's `make()` returns a relevant, flexible field tree (grouped SEO, nested groups like Salary range / Nutrition / Location, `json` escape hatches on Product/Documentation/API reference for open-ended data); everything stays editable after creation. [EXISTING]
- **Strong SEO group** (`seoFields()` in `schema-store.ts`, shared by templates + seeds): Meta title, Meta description, plus three smart fields — **Canonical URL** (a `useSiteDefault` toggle so it defaults to the page's own URL, override only to point elsewhere), a dynamic **OG image** (an `ogFallback` toggle so it falls back to the entry's cover/thumbnail when no custom image is set), and **Schema markup** (see below); page models add No index. [EXISTING]
- **Structured-data field with schema-type picker** (`schema` field type): instead of hand-pasting JSON-LD, the author picks a schema.org `@type` from a dropdown (`SCHEMA_TYPES`: BlogPosting, Article, NewsArticle, Product, Recipe, Event, Course, VideoObject, Organization, Person) and the builder shows the properties **auto-mapped from the model's fields** (`headline ← Title`, `image ← Cover`, `author ← Author`…); a **Custom** option still allows pasting raw JSON-LD. The model JSON exposes `{ type: "schema", schema: "<@type>" }`. Runtime field-value binding + head injection are the production wiring. [EXISTING]
- **Embed field** (`embed` field type): a field for pasting custom **HTML, CSS or JS** that renders live in a sandbox on the page — newsletter/marketing forms, third-party widgets, arbitrary embeds. Distinct from the entry BlockEditor's code-embed block: this is a typed *schema* field a developer exposes on a model. [EXISTING] (sandboxed runtime render is production wiring)
- **Searchable-by-default** (`defaultSearchable`/`isSearchableEligible`/`fieldSearchable` in `schema-store.ts`): the per-field Searchable toggle now defaults **ON for real content** and **OFF only for code** (`json` / `schema` / `embed`), and is offered on every indexable type — so authors opt OUT of a noisy field rather than tediously opting each content field in. The model JSON emits `searchable: true` on the effective-on fields. (The demo search index still reads the legacy CMS `schemas` via `TEXTY` + `fieldOff`; production reads this per-field flag — see §21.) [EXISTING]
- **Dynamic FAQ field + automatic FAQ schema** (`faq` field type — a differentiator): a developer drops a `faq` field, marketers author repeatable question/answer pairs (accordion), and the CMS **auto-generates valid FAQPage JSON-LD** from those questions via `faqPageJsonLd()` — no hand-written schema. The builder renders the accordion + a live "generated from the questions" JSON-LD preview and an "Auto-emit FAQPage JSON-LD" toggle (`emitFaqSchema`, on by default); the model JSON exposes `{ type: "faq", schema: "FAQPage" }`. Seeded on Blog post, Documentation, and Comparison page, plus a standalone **FAQ** block template. This closes the FAQ-rich-results gap for marketers and gives developers a typed field instead of loose repeaters. [EXISTING] (runtime rendering + head injection are the production wiring)
...
> **Stack note (§35):** the demo's Supabase email auth below is a **prototype**. Production standardizes on **Better Auth** (session email/password + OAuth) with **jose** for JWTs and **libsodium** for crypto — self-managed, no per-MAU bill, native SSO path. Also: the raw-`innerHTML` stored-XSS in the security backlog is closed by the stack's **isomorphic-dompurify** sanitizer (see §16).
...
- **Session model.** The production choice (§35) is **Better Auth** (session email/password + OAuth, jose for JWTs, libsodium) replacing the demo's Supabase login; either way: short-lived access JWT (~1h) + rotating refresh token, refresh-token reuse detection, server-side session registry so the `/account` sessions list and "sign out everywhere" become real. Prefer httpOnly cookies over the current localStorage storage to take tokens out of XSS reach (see 8.5).
...
[RECOMMENDED] Deployment shape (reconciled with §35): the **Hono/Bun monolith and workers run under podman on one Hetzner VPS**, alongside self-hosted Postgres, Valkey, and Typesense on the same box; **Cloudflare Workers** front the edge/api/forms/cron surfaces and a **Cloudflare Tunnel** exposes the VPS. Session verification is a JWT check (**Better Auth** + jose) at the gateway. The demo's Vercel deployment is a frontend preview host, not the production runtime.
...
> **Stack note (§35):** production standardizes the rich-text engine on **Lexical** (+ its headless build) as the framework under the block model, with **isomorphic-dompurify** as the sanitizer on any HTML boundary. That sanitizer is the concrete fix for the raw-`innerHTML` stored-XSS the audit flagged (the visual editor's inline rich mode and any paste path must run through it, including a `javascript:` href filter). The DocValue block model below is the storage contract regardless of which editor framework renders it.
...
- **[RECOMMENDED] Presence: build thin on Valkey (§35–36).** The simulation defines the contract: rooms per project with sub-locations (page path, section id, entry id, field name, block index), cursor positions, seat metadata, idle timeout. The stack carries **no Liveblocks line**, so presence rides a small in-house WebSocket gateway over **Valkey pub/sub + keyspace TTL heartbeats** — well-understood, no per-seat SaaS bill. The client hook surface (`useProjectPresence`, `peerLocationLabel`) stays, backed by Valkey instead of the RNG. (Liveblocks/PartyKit remain the escape hatch if the ws/heartbeat ops load ever outweighs a per-MAU bill — keep presence behind the store interface so the swap stays contained.)
...
| Usage rollups | cron hourly/daily | **[SIMULATED]** seeded numbers | Aggregate nginx/CDN logs (bandwidth), storage inventory, API gateway counters, credit ledger into `usage_metric` rows; feed Dodo metered billing |
...
| Email sends (invites, auth, billing) | event | **[SIMULATED]** except Supabase auth mail | Queue → **ZeptoMail** (@react-email templates) |
...
- **[RECOMMENDED] Tech (reconciled with §35).** The API is a long-lived Hono/Bun process on the VPS, so serverless-worker workarounds don't apply. Two sound, low-ops paths that fit the one-box + Cloudflare posture: (a) **Cloudflare cron Workers** for scheduled sweeps (publish, domain checks, digests, rollups) + event-triggered Workers — the team's current choice, zero extra stateful systems; (b) **Postgres-backed queues (Graphile Worker / pg-boss)** for durable in-process jobs (webhook delivery, media ingest, generators) — one database, transactional enqueue with the outbox pattern for free, and Valkey is already present for locks/rate-limits. Prefer (a) for crons and (b) for durable multi-step work; reach for a dedicated queue (BullMQ on Valkey) only if job volume genuinely outgrows Postgres.
...
> **Stack note (§35):** the payment processor is **Dodo Payments (Merchant of Record)**, not Stripe — Dodo absorbs global tax/VAT/compliance, which is the reason to pick it for a small team selling worldwide. A `DodoCheckout.tsx` shell already exists in the demo. The subscription-items structure described below (workspace item + per-site items + seat quantities + metered overage) is the **conceptual mapping**; implement it against Dodo's subscription/usage API rather than Stripe's. Everywhere the text says "Stripe," read "Dodo."
...
- **[RECOMMENDED] Pipeline + storage.** The team's current posture (§35) is **nginx-log-based analytics with no third-party service** — parse hosted-site access logs into per-project/page/referrer rollups. That is the right, cheap default at this stage: log-based counting needs no client JS and no analytics SaaS. Rollup jobs (section 27, i.e. Cloudflare cron Workers + VPS handlers) materialize daily/hourly aggregates; the dashboard API serves aggregates only. Keep the existing chart components and `DateRangePicker` — they bind to the same series shape `trafficSeries` emits. If per-pageview granularity for headless customers ever outgrows Postgres rollups, a columnar store (self-hosted ClickHouse) is the escalation — but do not buy Tinybird/ClickHouse before log-based rollups actually hurt.
...
> **Stack note (§35):** error tracking is **GlitchTip** (Sentry-compatible, self-hosted on the same Hetzner VPS) — the Sentry SDK works against it, so instrument with `@sentry/*` pointed at the GlitchTip DSN. Prefer self-hosted/OSS sinks (Grafana Loki/Tempo, Prometheus) over Datadog/Grafana Cloud to match the lean, one-box posture; the OTel guidance below still applies, just export to self-hosted collectors.
...
> **Reconciled with the real stack (§35):** the actual topology is **one Hetzner VPS + Cloudflare edge**, not Vercel + a managed-service fleet. The paragraphs below are updated to match; the migration/flags/secrets/rollback guidance underneath still holds.

**Environments.** `dev` (per-engineer, **podman compose**: Postgres + Valkey + Typesense + a local S3-compatible store/MinIO or an R2 dev bucket), `preview` (per-PR: an ephemeral app on the box or a scratch namespace with a seeded DB — the demo seed becomes the fixture), `staging` (prod-shaped, integrations in test mode), `production`. Do not conflate the product's "publish to staging" (a content state) with the org's staging (an infra tier).

**Topology.** The app is a **Hono/Bun monolith + workers on a single self-hosted Hetzner VPS** under **podman + nginx**, with Postgres + Valkey + Typesense + GlitchTip as containers on the same box, exposed through a **Cloudflare Tunnel**. **Cloudflare Workers** run the edge/api/forms/cron surfaces; **R2** holds media + artifacts; DNS, SSL, and Turnstile are Cloudflare. The demo's Vercel deployment is a preview host for the frontend only — production serves the dashboard from nginx/Workers, not Vercel. Scale-out is a bigger Hetzner plan first, then a second box / managed Postgres when one box saturates (§36–37).

**IaC.** Version the **Cloudflare edge** (Workers, R2, D1, DNS, Tunnel, Turnstile) and the **VPS provisioning** (podman quadlets / Ansible) as code; a single box does not need Terraform's full ceremony, but the Cloudflare side benefits from it. The customer-domain pipeline (verify TXT → issue cert via Cloudflare for SaaS / Let's Encrypt) is itself infrastructure-as-code — the demo's domain/SSL status flow is the UX spec for it.

**CI (GitHub Actions).** Pipeline per PR: install (bun, cached) → typecheck → lint → unit → build → contract tests → preview deploy → changed E2E → visual diff. Merge to main builds and pushes the podman image to the VPS (and `wrangler deploy` for the Workers) as staging; production is a manual promotion with a release tag. Trunk-based, no long-lived branches.

**DB migrations.** Versioned SQL migrations via **Drizzle Kit** (already the ORM's migration tool), forward-only, expand-and-contract for zero-downtime (add column → dual-write → backfill → cut reads → drop). Migrations run as a gated CI step before app rollout; every migration ships with a rollback note. Content schema changes (the customer-facing schema builder) are NOT DB migrations — they are versioned rows in a `schemas` table; never let customer models generate DDL.
...
## 35. Production Tech Stack

> **Reconciled 2026-07-12 against the team's own reference** ("Internal Tech Stack & Costs", Linear, updated 2026-07-08, plus the repos' `TECH_STACK.md`). The rows below are the stack Flowtrix **actually runs** across its nine repos (backend + published SDKs, dashboard, admin, docs, calculator, starters) — not a greenfield proposal. Where an earlier draft of this section suggested managed-cloud alternatives (Vercel-hosted API, tRPC/Fastify, managed Postgres/Redis, Stripe, Supabase Auth, Liveblocks, Resend), treat those as **superseded** by the choices here. The frontend advice still holds: the demo app is the spec and is kept as-is.

| Layer | Choice (in use) | Notes |
|---|---|---|
| Runtime | **Bun** (backend) · **Node 22** (frontends + starters) · TypeScript | Bun runs the API; Node runs the Vite/Next/Astro builds. |
| Backend API | **Hono 4** (+ hono-openapi, Scalar docs) · **Zod 4** | Not tRPC/Fastify/Nest. OpenAPI is generated by hono-openapi; Scalar renders the reference. Still a modular monolith (§12), just on Hono. |
| ORM / DB access | **Drizzle ORM + Drizzle Kit** on **PostgreSQL** (pgvector, Row-Level Security) · nanoid | Drizzle Kit owns migrations; pgvector powers embeddings/hybrid search; RLS is the tenancy fence. |
| Auth | **Better Auth** (session email/password + OAuth) · **jose** (JWT) · **libsodium** | Not Supabase Auth. The demo's Supabase login is the prototype; production standardizes on Better Auth (self-managed, no per-MAU bill, native SSO path). |
| Dashboard / Admin | **React 19 · Vite 8 · TanStack (Router/Query/Form/Table/Virtual) · Zustand + Zundo** | Matches the demo's TanStack base. Zustand + Zundo is the production state + undo layer — the demo's `useSyncExternalStore` stores map onto it. |
| UI | Tailwind CSS 4 · shadcn + Base UI + Radix · Lucide · cmdk · sonner · Framer Motion · dnd-kit | As the demo; `DESIGN_SYSTEM.md` is authoritative. |
| Rich text / content | **Lexical** (+ headless) · **isomorphic-dompurify** | Lexical is the production editor. isomorphic-dompurify is the sanitizer — **this is the fix for the raw-`innerHTML` stored-XSS flagged in §8 and §16.** |
| Search | **Typesense** (self-hosted on the VPS) + Typesense SDK · **Orama** on the docs site | Matches `SEARCH_PLAN.md`: scoped keys, per-field index, hybrid/vector. |
| Media / files | **Cloudflare R2** via AWS SDK v3 (S3 client) · **sharp** (transforms) · xlsx (SheetJS) · file-type | sharp resizes/reformats on the box — no Cloudflare Images/Cloudinary. R2 = zero egress. |
| Email | **ZeptoMail** (Zoho) + **@react-email** templates | Not Resend/Postmark. Pay-as-you-go, no base fee. |
| Billing | **Dodo Payments** (Merchant of Record) | Not Stripe. MoR absorbs global tax/VAT/compliance. `DodoCheckout.tsx` already exists in the demo. |
| LLM | **Anthropic API** (Claude Agent SDK) — platform key + per-workspace BYOK | Anthropic-only, not a multi-provider gateway. Tiers stay Lite/Balanced/Max mapped to Claude models. |
| Anti-spam | Cloudflare Turnstile | As specced in the forms builder. |
| Cache / sessions / rate-limit | **Valkey** (self-hosted on the VPS) | Redis-compatible fork; sessions, rate limits, ephemeral/pub-sub state. |
| Background jobs | **Cloudflare cron Workers** + in-process handlers on the VPS | No BullMQ/Temporal; scheduled Workers + handlers cover publish fan-out, rollups, reindex. |
| Edge | **Cloudflare Workers** (api / forms / cron / edge) · D1 (forms) · Tunnel · DNS + SSL · Turnstile | Workers Paid; Tunnel exposes the VPS. |
| Hosting | **One self-hosted Hetzner VPS** (Arm64) — podman + nginx + Postgres + Valkey + Typesense + GlitchTip | Everything stateful lives on one box. Scaling = a bigger plan, not new SaaS. |
| Observability | **GlitchTip** (Sentry-compatible, self-hosted on the VPS) | Errors; not Sentry SaaS / Datadog. |
| Analytics | **nginx-log based** — no third-party analytics | Parse access logs into the existing charts; search analytics from Typesense query logs. |
| Repo provisioning | **GitHub App** ("BetterCMS Deploy") + template repos | Free; provisions customer sites from the Next.js 15 / Astro 5 starters. |
| Social / export | Google OAuth + Sheets API | Social login + Forms→Sheets export. |
| Published packages | npm scope **`@bettercms-ai/`**: types · sdk · ui · next · astro · mcp · codegen · doctor · image-url | The MCP server (§15) ships as `@bettercms-ai/mcp`; headless SDK + framework adapters are public. |
| Testing / build | Vitest · Testing Library · tsup · esbuild · ESLint · Prettier · Changesets | — |

---

## 36. Build vs Buy

> Most of these are **already decided** in the stack above. This table records the reasoning so the tradeoffs stay legible — not to reopen them. The through-line is a deliberate lean-and-self-hosted posture: one box, Cloudflare at the edge, buy only what is genuinely painful to run (email deliverability, payments/tax, LLM).

| Capability | Decision | Reasoning |
|---|---|---|
| Compute / hosting | **Self-host on one Hetzner VPS** (podman + nginx) with Cloudflare Workers at the edge | A single Arm64 box runs Postgres + Valkey + Typesense + nginx + GlitchTip at this stage; scaling is a bigger plan, not a fleet of managed services. Keeps the fixed baseline low (§37). Revisit managed Postgres / multi-node only when one box is genuinely saturated. |
| Search | **Typesense self-hosted** (on the VPS) vs Typesense Cloud vs Algolia | `SEARCH_PLAN.md` rejected Algolia (proprietary, replica-per-sort, cost curve); scoped keys + per-field `searchable` + hybrid vectors assume Typesense semantics. RAM is the cost lever — the field opt-in keeps it down. Move to Typesense Cloud only if the ops burden bites. |
| Auth | **Better Auth** (self-managed) vs Supabase vs Clerk | Session + OAuth in-app with jose/libsodium: no per-MAU SaaS bill, no lock-in, native SSO path for the enterprise tier. The demo's Supabase auth is a prototype to be replaced, not kept; Clerk's DX doesn't justify a per-seat bill at scale. |
| Billing | **Dodo Payments (Merchant of Record)** vs Stripe | MoR absorbs global tax/VAT/compliance — the reason to pick it over Stripe for a small team selling worldwide. Cost is a % of revenue, not a fixed line. Build only the entitlement layer (`FEATURE_MATRIX`/`siteHas`); the two-layer-plan → subscription-items mapping in §28 still applies, just against Dodo. |
| Realtime presence/comments | **Build thin on Valkey + WebSocket** vs Liveblocks | The simulated presence engine (`presence-store.ts`, field/paragraph avatars, canvas cursors) is a precise spec, but the stack carries **no Liveblocks line**; presence, cursors, and run/notification streaming ride Valkey pub/sub + a small in-house WS layer. Nothing bought per monthly-active collaborator. Keep it behind the store interface so scope stays contained. |
| Media pipeline | **R2 + sharp** (own storage and transforms) vs Cloudinary/Mux | R2's zero egress matters for a bandwidth-metered CMS; sharp does resize/format on the box. No Cloudflare Images / Cloudinary / Mux until video delivery is actually real (the demo only stores video as library items). |
| Email | **ZeptoMail + @react-email** vs Resend/Postmark vs SES-raw | Pay-as-you-go (no base fee) with React Email templates for invites, the transfer accept-loop, workflow requests, and digests. Deliverability handled without SES-raw babysitting. |
| LLM | **Anthropic (Claude Agent SDK), platform key + BYOK** vs multi-provider gateway | A single provider keeps the call path simple; tiers (Lite/Balanced/Max) map to Claude models; BYOK shifts token cost to the workspace. A gateway abstraction is deferred until a second provider is a real requirement. |
| Analytics | **Build on nginx logs** vs PostHog/Plausible/ClickHouse | No third-party analytics SaaS; parse access logs into the existing charts. Search analytics come from Typesense query logs. Own the pipeline; the analytics pages are the presentation spec. |
| Observability | **GlitchTip self-hosted** vs Sentry SaaS / Datadog | Sentry-compatible SDK running on the same VPS — error tracking without the SaaS bill. |

---

## 37. Cost Estimation

> **Corrected against the team's real architecture (2026-07-12).** An earlier draft modeled a managed-cloud stack (Vercel + Fly/ECS + managed Postgres/Redis + Liveblocks + Stripe + Supabase Auth) whose infra bill would have climbed from the high hundreds into the tens of thousands of dollars a month with scale. **That does not describe what Flowtrix runs.** The actual architecture — one self-hosted VPS + Cloudflare edge + usage-metered APIs — has a **low, flat fixed monthly baseline**, roughly an order of magnitude or more below the managed-cloud model, with everything else scaling on real traffic. (Exact per-line dollar figures live in the team's internal cost reference, not this document.)

### Fixed baseline

A single self-hosted VPS carries almost the entire fixed cost: **Postgres, Valkey, Typesense, nginx, and GlitchTip all run in podman on that one box**, so there are no separate managed-service line items — their cost *is* the VPS. On top of it sit only a small Cloudflare Workers floor and amortized domain registration. Cloudflare R2, D1, Tunnel, Turnstile, DNS/SSL, the GitHub App, and Google OAuth + Sheets all sit inside free tiers. **Net: a low, flat monthly baseline; scaling up means a bigger VPS plan, not a new stack of subscriptions.**

### Usage-based lines (scale with traffic, not fixed)

- **Anthropic API** (the in-dashboard agent) — billed per token; the single most volatile line. **BYOK shifts it to the workspace**, and the credit markup must clear platform-key cost.
- **Cloudflare R2** — storage + operations with **zero egress fees**; small at current scale.
- **ZeptoMail** — pay-as-you-go transactional email, no base fee.
- **Dodo Payments** — a percentage of revenue (Merchant of Record absorbs tax/VAT/compliance); this is COGS on sales, not infrastructure.

**The takeaway is unchanged: team cost dwarfs infrastructure.** Against a low fixed baseline, infra is a rounding error next to engineering burn — optimize for build speed, not cloud bills. The usage lines (Anthropic tokens above all) only bite once real customer traffic exists, and BYOK + the credit markup are the levers that keep them cash-positive. The scaling story is a bigger VPS (and eventually a second box / managed Postgres) — not the managed-service ladder the earlier draft assumed.
...
Total to GA (≈7 months): **≈ $650–700k engineering + a negligible cumulative infra baseline** (the flat monthly VPS + edge cost over 7 months, plus modest usage). Team cost dwarfs infra by orders of magnitude at this stage; optimize for build speed, not cloud bills.
...
|---|---|---|
| Foundations: auth + tenancy | Postgres schema v1 (Drizzle), Better Auth session/JWT middleware (jose), workspace/project/member/seat model, RLS, view-as clamps server-side | 6 |
...
| Media | R2 upload (signed URLs, AWS SDK v3), library metadata, transforms via sharp, crop persistence, picker wiring | 4 |
...
| Billing | Dodo Payments (MoR) subscriptions (two-layer), seats, AI-credit metered usage, entitlement sync to `FEATURE_MATRIX`, invoices/payment surfaces, dunning | 6 |
| Collab / realtime | Presence + comments on Valkey pub/sub + WebSocket, notification socket, run streaming (SSE) | 5 |
...
**Day 60:** entry block editor + schema builder on the unified schema service; workflows + typed requests + notifications (email via ZeptoMail); Dodo Payments subscriptions live in test mode with entitlements driving the existing gates; agent runs execute against the real Claude Agent SDK for 2 skills (draft, backfill-SEO) with server-side undo journal; private alpha with 5 design partners.

**Day 90:** all 6 skills + both generators real; credits metering + governance enforced server-side; Typesense indexing on publish + playground live; MCP endpoint with scoped keys; forms public endpoint + Turnstile; presence (Valkey + WebSocket) on the visual editor; E2E suite covers the 12 critical flows; beta gate review against SLOs.
...
| CNT-6 | Publish/schedule/staging pipeline | Server-side state machine (draft/published/modified/scheduled/archived + staged), snapshot on publish, scheduler (Cloudflare cron Worker + Postgres claim-lock sweep), per-item publish parity with PublishMenu. | Publish < 10s p95 to delivery API; schedule fires ±60s; snapshot retrievable; unpublish/cancel paths | 4 | CNT-2, OPS-2 |
| OPS-2 | Job infra + worker skeleton | Cloudflare cron Workers for scheduled sweeps + Postgres-backed queue (Graphile Worker / pg-boss) for durable jobs, DLQ pattern, retry policy, job metrics exported (Valkey available for locks). | Jobs retry w/ backoff; DLQ alert fires in staging test; dashboard panel | 2 | FND-2 |
...
| BIL-1 | Dodo Payments foundation + entitlements | Dodo (Merchant of Record) customers/subscriptions for workspace + site plans (test mode), webhook sync into an entitlements table read by the existing `siteHas`/`FEATURE_MATRIX` gates. | Plan change in Dodo flips gates in-app < 1 min; webhook replay-safe; seat quantity syncs | 4 | FND-5 |
| AI-1 | Claude Agent SDK + runs persistence | Anthropic-only agent runner (tier→Claude-model map hidden per product rule, cost capture per call), `agent_runs`/`proposals`/`undo_ops` tables, run state machine server-side, SSE streaming to the existing dock UI. | One skill (backfill-SEO) runs real end-to-end: plan → approve → apply → **undo works**; credits + audit recorded; retry/backoff tested | 5 | CNT-4, OPS-2 |
| OPS-3 | Observability baseline | OTel tracing + pino logs + GlitchTip (Sentry-compatible; replacing `reportLovableError` call sites) + Grafana dashboards for the §32 SLIs; alert rules for availability class. | Trace per request joinable by requestId; publish-latency + job-DLQ alerts firing in staging chaos test; GlitchTip receives boundary errors with release tags | 3 | FND-6 |

(Next in queue, for context: SRC-1 Typesense provisioning + index-on-publish, AI-2 remaining skills, EDT-2 presence on Valkey + WebSocket, BIL-2 credit metering, MCP-1 endpoint + scoped keys.)
...
2. **Self-host story.** Is BetterCMS ever self-hostable/open-core? Decides whether services stay portable (the stack is already lean and self-hosted — one Hetzner VPS, no managed-SaaS lock-in) and whether the license conversation starts now. [Default: cloud-only through GA; keep the podman-compose dev parity as a cheap option-preserver.]
3. **AI provider defaults + BYOK policy.** Which Claude models back Lite/Balanced/Max, is BYOK available below Enterprise, and do BYOK runs still consume (discounted) credits for margin? The demo says BYOK runs bill the key and credits=0 — confirm that's the commercial intent. [Default (per §35): **Anthropic-only via the Claude Agent SDK** — no multi-provider gateway; tiers map to Haiku/Sonnet/Opus; BYOK Pro+, credits=0 as specced. Add a second provider only if a concrete need appears.]
...
| **Realtime scope creep** (presence spec is rich: cursors, per-paragraph avatars, field locks → team builds CRDT infra) | High | Medium — quarters lost to infra without product delta | Ship presence as a thin Valkey + WebSocket layer (§36), not CRDTs; hard rule: no in-house OT/CRDT before GA; keep presence behind the store interface so a later Liveblocks/Yjs swap stays contained |
