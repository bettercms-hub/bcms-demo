...
- **Strong SEO group** (`seoFields()` in `schema-store.ts`, shared by templates + seeds): Meta title, Meta description, Canonical URL, **OG image** (picked from the media library in the entry editor), and a **Schema markup** `json` escape hatch for extra JSON-LD; page models add No index. [EXISTING]
