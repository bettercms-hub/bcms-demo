...
- **New model** → template gallery (`SCHEMA_TEMPLATES`, 19 ready-made shapes: Blog post, Testimonial, Author, Case study, Glossary term, White paper, Product, Job posting, Event, Documentation, API reference, Course, Recipe, Video, Podcast episode, Music track, Portfolio project, Webinar, Comparison page, or blank) → `modelActions.add(projectId, model)` with `newModelId()`. Each template's `make()` returns a relevant, flexible field tree (grouped SEO, nested groups like Salary range / Nutrition / Location, `json` escape hatches on Product/Documentation/API reference for open-ended data); everything stays editable after creation. [EXISTING]
